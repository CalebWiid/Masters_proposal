...
\subsection{Googles MediaPipe Hands}
...
Following on from this research, \cite{Shahi2024} developed a system that also learns to recognise a gesture from a single observation. The authros of this research present a vision-based hand gesture recognition system that enables gesture customization from a single demonstaration. The system proposed by \cite{Shahi2024} developed a end-to-end framework that supports a wide range of gestures, including both static and dynamic, one and two-handed gestures as well as gestures from multiple viewpoints. The method combines hand keypoint extraction, a graph transformer architecture, and a few-shot learning (FSL) stratagy besed on model-agnostic meta-learning (MAML) to rapidly adapt to new gesture classes from minimal data. Hand pose estimation is used to extract skeletal joint coordinates, which are processed by a graph transformer to capture both the spatial relatiobships between the joints and the temporal motion patterns. To further enhance learning from limited data, a meta-augmentation technique is introduced to increase task diversity by generating a large number of additional gesture combinations to build a greater dataset. Unlike many prior works, the system also incorporates a background class to distinguish intential gestures from irrelevant hand movements, improving real world robustness. Figure \ref{fig:FSL_overview} gives a good overview of the system \citep{Shahi2024}. 
...
\cite{Shahi2024} evaluated the  system through a user study involving 21 participants and 20 gesture classes. The results showed an average accuracy of 95~\% with only one demonstration per gesture. This shows that the system outperforms baseline approaches such as fine-tuning, nearest neighbours and random forest classifiers. Additionally, the authors demonstrate the practicallity of their approach via three real-wolrd applications, including a gesture design tool, gesture-based productivity interfaces, and mixed reality interaction systems.  
...
A numebr of companies have since developed similar products, most notably, Meta \citep{MetaEMGWearable} and Mudra Wearables Inc \citep{MudraBand}. The neural band developed by Meta is used to allow users to control the Meta Ray-Band display as well as the Orion ecosystems \citep{MetaEMGWearable}. On the other hand, the Mudra band was designed for a more general perpose. This band allows users to control a wide range of devices such as computers and smart TV's, and can eliminate the need for a mouse, keyboard or touchscreen \citep{MudraBand}. Both the Meta band and the Mudra band can be seen in Figure \ref{fig:combined_neural_bands}.
