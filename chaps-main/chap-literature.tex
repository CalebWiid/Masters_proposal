...
This chapter will discuss the current state of the art technology that incorporates gesture recognition to control a drone movement. There are a number of different techniques to implement this and they will be discussed. Additionally different learning algorithims used to recognise and catagorise gestures will be discussed. 

\section{Types of Gesture Control}
\subsection{Image-Based vs Non-Image-Based Gesture Recognition}
According to \cite{Khaksar2023} there are two general approaches when it comes to gesture recognition and data acquisition: image-based and non-image-based approaches. The image-based approach can be further categorised into three main subcategories which include depth camera detection, stereo camera detection and single camera detection as shown in \ref{fig:gesture_sensors}. This approach makes use of cameras to capture a gesture and then using feature extraction transforms this information into usable data that a system can use.

The non-image based approach on the other hand makes use of sensors to capture a gesture. These sensors are catogorised into glove devices and band devices. Typically these devices make use of Inertial Measurment Units (IMUs) in combination with flex sensors to capoture the movement of a hand or arm \hl(insert source). These deviceces are typically more robust to environmenteal factors copared to the image-based approach, however the recent trend has been to move to image-based approachces as they are less intrusive and more userfriendly \hl{source}. Image-based approaches also make use of the camera already mounted on the drone, thus making it more cost effective and easier to implement \hl{source}. 
...
The sensors mentioned above are used to capture the movementes of a hand gesture, however, they do not interperate the meaning of the gesture. In order to do this, a gesture language is needed. A gesture language is a dataset that matches a gesture to a meaning \citep{Patrona2021}. There are a number of different gesture languages that have been used in the past for general gesture recognition, and have been applied to both manned and unmanned aerial vehicles. According to \cite{Patrona2021}, the following are some of the know gesture languages: 
\begin{itemize}
...
\end{itemize}
These datasets are very helful when it comes to gesture control of drones, however they are limited in that they require the user to learn the specific gesture languages and they do not allow for personalisation of the gestures. This is a major limitation as it does not allow for users to use their own natural gestures, and it also does not allow for users to adapt the gesture language to their specific needs and preferences. This is a gap that this project aims to address by allowing users to create and personalise their own gestures for controlling the drone. 
...
Before the data can be used it needs to undergo a pre-processing phase, which includes a signal trimming phase and a Normalization phase. The signal trimming phase is used to remove the first and last 3 \% of the signal data to eliminate electrical noise or the "transistion" movements of the hand at when the user begins or ends a gesture. The normalisation phase is used to normalize the raw analog-to-digital vaklues into a consistent dataset to ensure that different users produce recognizable data petterns. 

Following this pre-processing, the framework employed machine learning inference to classify the processed data into one of 25 distinct hand gestures.The researchers evaluated four specific machine learning algorithms to determine the most effective model for real-time flight control: Decision Tree (DT), Naive Bayes (NB), Support Vector Machine (SVM), and K-Nearest Neighbors (KNN). To ensure statistical significance, the performance of these classifiers was verified using a dataset of 49,000 datapoints collected from 20 participants aged between 22 and 63. The experimental results revealed critical trade-offs between classification accuracy and computational speed. SVM was found to be the most precise model, achieving a mean accuracy of 98.02\%, though it required the longest training time at approximately 20,666.7 ms. For real-time piloting where low latency is paramount, KNN proved superior, delivering the fastest test time of 1.22 ms. Decision Tree offered the most efficient training phase at 133.1 ms, while maintaining a competitive accuracy of 96.96\%.

Once a gesture is recognized, the system utilizes a task scheduling algorithm to translate the identified class into pre-defined UAV flight commands. This multi-mode command structure allows for complex 3D navigation; for instance, specific hand orientations and finger flexions can trigger simultaneous movements such as ``Forward Up'' or ``Turn Left Down''. This glove-based approach offers a significant engineering advantage over vision-based systems, as it remains robust against variable ambient lighting and line-of-sight obstructions that typically hinder camera-reliant gesture recognition. The system's high accuracy and millisecond-level response times demonstrate its viability for high-precision human-UAV interaction in complex environments. One downside to this design is that it requires the user to wear a glove, making the device "intrusive and constraining fo the user" \citep{Gio2021}. 

\subsection{Skywriter HAT sensor}
\cite{Mehta2023} made use of a near-field elctric field sensor (Skywriter HAT sonsor shown in Figure \ref{fig:skywriter_hat}) to control the roll, pitch and yaw of the drone. The sensor detects a change in a magnetic field when a conducticve object (such as a hand) enters in the field. The change in magnetic field is converted into a digital signal that represented the x, y and z coordinate values. These values were used by a Raspberry Pi that coonverts these alues to the roll, pitch and yaw to control a Crazyflie drone. An Extended Kalmen Filter was used to smooth the signals noise and to combine the information into one reliable estimate of the drone's position, velocity, and orientation at any moment.
...
The authors showed some success with this design hwoever there were real limitations and concerns. The first was that the Skywriter sensor was noisy and the they were unable to completly isolate which acis was moving. This also cause the drone to drift when the hand was held still.    

\subsection{LEAP Motion Sensor}
\cite{Sarkar2016} describes the Leap Montion Sensor as a gesture recognition device that uses two monochromatic infrared (IR) cameras and three infrared LEDs to track infrared light. By using stereoscopy from its two cameras, the Leap reduces errors and achieves highly precise tracking of hand and finger movements. The sensor can track hand motion up to a distance of about 1 meter directly above it, creating a hemispherical area above the sensor. Once the hand enters the area, the sensors detects movement and using the LEAP Software Development Kit (SDK), advanced Computer Vision algorithems run in order to exctract raw sensor data. Figure \ref{fig:Leap_sensor} \citep{Sarkar2016} shows the motion sensor as well as the hemispheroical section where a hand movement can be detected. 

\begin{figure} [htbp]
    \centering
    \includegraphics[scale=1.0]{figs/Leap_sensor.png}
    \caption{Leap motion sensor and the hemispherical area}
    \label{fig:Leap_sensor}
\end{figure}


The Leap motion sensor has been used by \cite{Sarkar2016} as a ground station to control the movements of an AR DRONE 2.0. However this sensor was only used to control the pitch, yaw and roll of the drone and it was not used to communicate high level commands to the drone.  

\subsection{Microsoft Kinect}
\cite{Gio2021} developed a system that uses the Microsoft Kinect to record full body gestures to control a drone. The Microsoft Kinect is used as a depth-sensing device that captures the full body movements by constructing a real-time 3D skeletal model of the user. This model tracts the posiotions of the users jouints (eg. shoulders, elbows, spine and hips) which allows the system to track the posture and movement of the user. The gesture recognitions is done by computing the angles between the joints rather than relying on the absiolute positions. These calculated angles are compared to experimentally defined thresholds to determine which gesture is being peformed. This allows for gestures to be interperated, regardless of the users's body size or the distance from the sensor, making the system more felxible for users. Figure \ref{fig:Kinect_overview} shows a flow chart of how the system interacts with each subsystem \citep{Gio2021}.
...
The Kinect streams the joint position data to a computer, where the motion recognition software evaluates the geometroc telationships between the various joints. For examples the torso inclination, or the arm position is translazted into angular values, and specific combinations of these angles correspond to drone commands such as move forward, sideways or rotate. Figure \ref{fig:Kinect_commands} shows the commands and associated gestures that \cite{Gio2021} has developed.
...
The use of angle-based recognition also improves robustness  by reducing sensitivity to environmental factors and positioning constraints. Experimental validation showed that the system could successfully recognise gestures for all participants, demonstrating reliable performance across different users. 

One key contribution of this work is its implementation of spped control, wich is achieved using the amplitude of the user's movements. Instead of gestures acting a simple on/off commands, the magnitude of the body motion is mapped to the drones velocity. Larger movements correspond to a higher speed command where as smaller, less pronounced movements result in lower speed commands. This form of control was achieved using a continuous linear mapping between the gesture amplitude and the speed of the gesture. Figure \ref{fig:Kinect_speed_control} shows how the amplitude and speed of the gestrure is used to control the speed of the drone.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[scale=0.43]{figs/kinect_speed1.png}
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[scale=0.47]{figs/kinect_speed2.png}
...
Figure \ref{fig:image1} shows the angle $\varphi ' $ is greater than $\varphi$ which results in the lateral speed $v'_{lateral}$ of the drone being greater than the lateral speed $v_{lateral}$ of the drone. The following forulas are used to caluclate the gradient anf the equation for the linear regression line shown in \ref{fig:image2}:
...
An additoinal feature that this system has is a graphical user interface (GUI) designed to enhance the user interaction. The interface displays real-time video of the drones onboard camera, along with additional information such as battery status and a live view of the skeletal tracking, allowing users to confirm that their gestures are correctly recognised. An interactive, gesture-controlled menu is integrated into the GUI, enabling users to select different flight modes and settings without needing a separate controller. This design choice further emphasizes the hands-free nature of the system and enhances the overall user experience. an example of the GUI is shown in Figure \ref{fig:Kinect_GUI}. 
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[scale=0.5]{figs/GUI_menu.png}
        \caption{GUI menu }
        \label{fig:GUI_menu}
    \end{subfigure}
    \hfill
    \begin{subfigure}[b]{0.45\textwidth}
        \centering
        \includegraphics[scale=0.5]{figs/Depth_map.png}
        \caption{Depth map of the user with the skeletal tracking overlay}
        \label{fig:Depth_map}
    \end{subfigure}
    
    \caption{Graphical user interface for the Kinect system}
    \label{fig:Kinect_GUI}
\end{figure}
...
One of the most common approaches to gesture recognition is to use Googles MediaPipe Hands (MPH) software. \cite{Khaksar2023} has used this software to develop a quadcopter control system as an alternative Human-Computer Inteaction (HCI) by focusing on replacing conventional drone control methods with a Hand-Gestuer Recognition (HGR) interface. The aim of this system was to deevelope and evaluate a HGR system capable of provifing an intuative, low-cost, and reliable mens of controllign a quatcopter without the neeed for specialised or wearable hardware. 

The cental element of this ssystem is the MediaPipe Hands (MPH) framework.The authors descibe the MPH framework as an on-device real-time hand identification software designed to operate using a single RGB camera. 
MediaPipe Hands operates using a two-stage machine-learning pipeline: first, a palm detection model identifies regions of interest within an RGB image, and second, a hand landmark model estimates a 21-point skeletal representation of the detected hand. Figure \ref{fig:MediaPipe_hand} shows the landmarks \citep{Taylor2025}.
...
Using these equation a matrix, that corresponds to specific gestures is constructed and can be seen in Figure \ref{fig:Gesture_matrix} \citep{Taylor2025}.
...
The drone can also be controlled by moving the hand closer or further away from the camera, which will control the distance of the drone from the user. This is done by moving both hands closer togethert to bring the droen closer, and moving the further apart to move it further way as seen in Figure \ref{fig:DJI_Dist} \citep{DJI_Neo2_Manual}.  
...
The The drone has additional festures however they are not controlled with a gesture, but instead using buttons on the drone itself. Soem of these additonal features include ActiveTrack, Spotlight an Quickshots. The Quickshots feature on DJI drones are specific cinimatic shots that have a set flight path. These include: 
...
One of the limitations that this drone has is that the gesture control can only be used to position the drone, and all additonal fucntions needs to be set on the dron itself or by using a remote controller. Therefore this project will aim to develop a system that allows for more complex commands to be given to the drone using gestures, such as taking a photo, recording a video, or activating specific flight modes. 
