...
\begin{itemize}
    ...
    \item Control basic drone functions using gestures.
    \item Learn unique user-defined gestures and assign them to functions.
    \item Perform complex photography-specific functions.
    \item Manage similar or ambiguous gestures reliably.
\end{itemize}

\section{Motivation}

Gesture-based drone control presents a compelling alternative to traditional manual controllers. By leveraging natural human motor skills, it aligns interaction with instinctive movement patterns, reducing the complexity associated with conventional control interfaces \citep{Trinitatova2025}. This translates directly into improved accessibility as users without prior piloting experience have been shown to operate drones safely and intuitively, particularly when combined with features such as obstacle avoidance and 3D trajectory mapping \citep{Taylor2025}. Gesture-based control also reduces dependency on specialised hardware such as dedicated radio controllers, broadening the range of contexts in which UAV technology can be practically deployed \citep{Zhao2018}.
...
A photography drone was chosen as the platform because it offers a wide and complex range of tasks, making it an ideal testbed. Although the underlying system could be applied to many other domains; such as agriculture, search and rescue, or surveillance; the photography use case was selected as it presents the greatest variety of distinct functions. If the system performs effectively in this context, it is well-positioned to be adapted to other industries.
